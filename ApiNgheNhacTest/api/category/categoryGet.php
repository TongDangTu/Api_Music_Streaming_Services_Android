    ...
    if (empty($conn) == false) {
        ...
        try {
            if (empty($_GET['id']) == false) {
                $str_id = $_GET['id'];
                $stmt = mysqli_prepare($conn, "SELECT * FROM category WHERE id=? LIMIT 1");
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $data_response['message'] = "Lấy thể loại thành công";
                    }
                }
                else {
                    $data_response['status'] = false;
                    $data_response['message'] = "Không có thể loại cần lấy";
                }
            }
        }
        catch (Exception $ex) {
