    ...
    if (empty($conn) == false) {
        ...
        try {
            ...
            $query = "SELECT song.id AS id_song, song.name AS name_song, song.playedTime AS playedTime, song.linkSong AS linkSong, song.linkPicture AS linkPicture,
                    ...
                    FROM song INNER JOIN song_playlist ON song.id = song_playlist.id_song
                                LEFT JOIN song_artist ON song.id=song_artist.id_song
                    ...
                    WHERE song_playlist.id_playlist = $id_playlist";
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $data_response['message'] = "Lấy danh sách yêu thích thành công";
                }
            }
            else {
